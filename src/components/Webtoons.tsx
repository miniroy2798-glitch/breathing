...
import { webtoons as initialWebtoons } from '../data';
import { BookMarked, Plus, Pencil, Trash2, Save, X, Sparkles, GripVertical, ChevronUp, ChevronDown, ArrowUpDown } from 'lucide-react';
...
export function Webtoons() {
  ...
  const [addStatus, setAddStatus] = useState('Reading');
  ...
  useEffect(() => {
    const saved = localStorage.getItem('app_webtoons_v3');
    let loadedItems: Webtoon[] = [];
    if (saved) {
      try {
        loadedItems = JSON.parse(saved);
      } catch (e) {
        loadedItems = initialWebtoons;
      }
    } else {
      loadedItems = initialWebtoons;
    }
    // Ensure all have ranks assigned 1..N based on index if not present
  ...
  const saveToStorage = (updatedList: Webtoon[]) => {
    ...
    setItems(sortedAndRanked);
    localStorage.setItem('app_webtoons_v3', JSON.stringify(sortedAndRanked));
  };
  ...
  const handleDragEnter = (e: React.DragEvent, index: number) => {
    ...
    // Re-assign ranks right away for layout responsiveness
  ...
  const handleAddSubmit = (e: React.FormEvent) => {
    ...
    sorted.splice(insertIndex, 0, newItem);

    saveToStorage(sorted);

    // Reset Form
    setAddTitle('');
    setAddStatus('Reading');
    setAddNote('');
    setIsAdding(false);
  };

  const startEdit = (item: Webtoon) => {
    setEditingId(item.id);
    setEditTitle(item.title);
    setEditStatus(item.status || 'Reading');
    setEditNote(item.note || '');
    setEditRank(item.rank || 1);
  };
  ...
  const handleEditSubmit = (e: React.FormEvent, id: string) => {
    ...
    const remaining = items.filter(item => item.id !== id).sort((a, b) => (a.rank || 0) - (b.rank || 0));
            ...
            onClick={() => {
              if (isRearranging) {
                ...
                executeSecuredAction(
                  ...
                  "Verify password to sort your Webtoons"
                );
              }
            }}
              ...
              if (isAdding) {
                ...
                executeSecuredAction(
                  () => {
                    setIsAdding(true);
                    setIsRearranging(false);
                    setAddRank(items.length + 1);
                  },
                  "Authenticate to Add Webtoon",
                  "Verify password to add a new Webtoon"
                );
              }
            }}
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <AnimatePresence initial={false}>
            {items.map((webtoon, idx) => (
              ...
              >
                {editingId === webtoon.id ? (
                  /* Editing Mode Form */
                  <form onSubmit={(e) => handleEditSubmit(e, webtoon.id)} className="w-full space-y-4">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-2.5 flex-wrap gap-y-1.5 mb-1.5">
                          <span className="px-2 py-0.5 bg-amber-50 rounded text-[10px] font-extrabold text-amber-700 border border-amber-100/50 shrink-0">
                            Rank #{webtoon.rank || idx + 1}
                          </span>
                          <h3 className="text-lg font-semibold text-stone-900 leading-tight">{webtoon.title}</h3>
                          {webtoon.status && (
                            <span className={`px-2.5 py-0.5 text-[10px] rounded-full font-semibold tracking-wide ${
                              webtoon.status === 'Completed' ? 'bg-green-100 text-green-700' : 
                              webtoon.status === 'Reading' ? 'bg-blue-100 text-blue-700' :
                        ...
                        {webtoon.note && <p className="text-xs text-stone-500 italic">"{webtoon.note}"</p>}
                      </div>
                    </div>

                    {/* Actions (visible on hover and always on mobile) */}
                    {!isRearranging && (
                      deletingId === webtoon.id ? (
                        ...
                        <div className="flex items-center space-x-1 opacity-80 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-200 self-center md:self-start shrink-0">
                          <button
                            onClick={() => executeSecuredAction(
                              ...
                              `Verify password to edit "${webtoon.title}"`
                            )}
                          ...
                          <button
                            onClick={() => executeSecuredAction(
                              () => setDeletingId(webtoon.id),
                              "Authenticate to Delete",
                              `Verify password to delete "${webtoon.title}"`
                            )}
        ...
        {items.length === 0 && (
          <div className="text-center text-stone-400 py-16 border-2 border-dashed border-stone-200 rounded-2xl bg-white italic">
            Your collection is empty. Add your favorite Webtoons above!
          </div>
        )}
      </div>
